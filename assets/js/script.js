...
function createPlaylistDOM(playlistData) {
   ...
   document.getElementById("container").after(ulElement);
}

async function getPlaylistData() {
   // Fetches list of songs that match end-users' selected genre
   const response = await fetch("https://api.spotify.com/v1/recommendations?limit=50&market=US&seed_genres=" + genre, {
      headers: {
...
function createGenreDOM() {
   ...
   selectElement.className = "genre-list-group";
   selectElement.name = "genre-list-group";
   selectElement.id = "genre-list-group";
